...
// ─── events ──────────────────────────────────────────────────────────────────
//
// Slug uniqueness (case-insensitive), FKs to auth.users, updated_at trigger,
// and RLS policies are all hand-written in the sibling migration file.
...
export const events = pgTable(
  ...
  (t) => [
    ...
    check(
      'events_ticket_url_format',
      sql`${t.ticketUrl} is null or ${t.ticketUrl} ~ '^https?://'`,
    ),
  ],
);
