...
- **🚫 NEVER push to the remote/production Supabase.** The linked project `raigylcyufzyyykjuvqw` ("District Pour Haus") is **production**. `supabase db push`, `--linked`, `supabase link`, and any command hitting `*.supabase.co` / `pooler.supabase.com` are **forbidden** and hard-blocked by `.claude/hooks/block-remote-db-push.py`. Production DB changes happen only when the **user** runs an explicitly-approved deploy. If a change must reach production, STOP and ask the user.
- **🚫 NEVER apply migrations or reset the database — not even locally.** No `npm run db:reset`, `supabase db reset`, `drizzle-kit push`, or `npm run db:seed` without explicit user approval. Agents and skills create migration files only and report the path; the **user** applies every migration manually. (A local reset wipes auth users and saved integration credentials.)
